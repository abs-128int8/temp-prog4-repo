...
def format_number():
    return  float("".join(var["front"] or ["0"]) + "." + "".join(var["back"] or ["0"]) )
...
def calculate_click():
    ...
    if var["operator"] in BINARYOP:
        var['result'] = eval(str(var['x_val']) + var['operator'] + str(var['y_val']))
    elif var["operator"] in FUNC:
        var["result"] = eval("math." + var["operator"] + "(" + str(var["x_val"]) + ")")
    update_display(var["result"]) 
